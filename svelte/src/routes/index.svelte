<script lang="ts">
	...
	onMount(async () => {
		...
		// Get current characters layers by using each first asset per layer
		Object.keys(layerOptions).forEach((layer) => {
			currentCharacter[layer] = layerOptions[layer][0];
		});
...
<section>
	{#if done}
		{#await getLayeredSvg({ layers: currentCharacter })}
			<p>...</p>
		{:then data}
			<svg
				id="token"
	...
	{#each layerOrdering as category}
		...
		{#each layerOptions[category] as layer}
			{#await getLayeredSvg({ layers: { ...currentCharacter, [category]: layer } })}
				<p>...</p>
			{:then data}
				<svg
					id="token"
