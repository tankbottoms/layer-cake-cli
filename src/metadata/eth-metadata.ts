...
import { random_birthday, Stat } from './stats'
...
export function generate_ethereum_metadata(asset: Asset) {
   ...
   const tags: string[] = [];
    ...
    switch(the_project.config.name) {
      ...
      default: { // test
        logger.warn('using test project stats for metadata')
        stats = test_stats;
        break;
      }
    }
   ...
   const boost_attributes = [];
    ...
    let md:IHash = {
      ...
      date: i.drop_date,
      tags: tags,
      background_color: (i.background_colors !== undefined) ? i.background_colors[random(i.background_colors.length)] : 'FFFFFF',
      language: `en`,
      mimeType: `image/png`,
